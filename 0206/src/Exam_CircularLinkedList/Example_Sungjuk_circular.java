package Exam_CircularLinkedList;
import java.util.Scanner;

/*
[이전 문제] 데이터 입력, 출력, 검색, 삭제 기능을 single linkedlist 로 구현하시오
...
*/
//[문제] 단순연결리스트 -> 원형 연결 리스트로 변경
public class Example_Sungjuk_circular {
...
//			temp.link = newNode; // newNode -> temp.link에 대입 (마지막 노드와 연결)
		...
		public void printList() { //전체 데이터 출력, 데이터 없을 때 메시지
			...
			do{
				Sungjuk obj = temp.getData();	//성적 객체 불러오기
				obj.printData();
		...
		public void deleteNode(Sungjuk data) {  //삭제
			...
			//리스트에 노드가 한 개만 있는 경우(수정)
			if(this.head.link == head) {	
				temp = this.head;
				obj = temp.getData();
				if(data.getBunho() == obj.getBunho()) 
				{	//첫번째 노드의 번호와 일치하는 경우
			...
			//리스트에 노드가 여러 개 있는 경우
				...
				if(data.getBunho() == obj.getBunho()) //노드가 여러 개일때 첫 번째 노드가 삭제할 노드인 경우
				{
					...
					//루프를 돌릴 수 있게 맨 마지막 노드를 찾는 과정이다
					while(temp.link != head) //수정
						temp = temp.link;	//수정
					temp.link = head.link;	//수정 head가 가르키는 link가 맨 마지막 노드가 
					this.head = this.head.link;	//후속 노드의 주소를 선행 노드의 링크에 저장
				}
					...
					do{
						...
						if(data.getBunho() == obj.getBunho()) //중간 노드 삭제 과정
						{
							System.out.println("\\n삭제가 되었습니다.\\n");
