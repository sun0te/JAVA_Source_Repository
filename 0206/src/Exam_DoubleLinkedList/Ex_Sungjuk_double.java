package Exam_DoubleLinkedList;
import java.util.Scanner;

//[문제] 이중 연결 리스트로 변경
public class Ex_Sungjuk_double {
...
class LinkedList {
		...
		public void deleteNode(Sungjuk data) {  //삭제
			...
			if(this.head.rlink == null) {	//리스트에 노드가 한 개만 있는 경우
				...
				{
					System.out.println("\n삭제가 되었습니다.\n");
					this.head = this.head.rlink;// 후속 노드(두 번째 노드)의 주소를 헤드 노드에 저장
					head.llink = null;
					return;
				}
				else {	//노드가 여러 개일때 첫 번째 노드가 아닌 노드를 삭제할 경우
					temp = head.rlink;	//두 번째 노드의 주소 저장
					while(temp != null) {
						obj = temp.getData();
						if(data.getBunho() == obj.getBunho()) 
						{
							...
							{
								temp.llink.rlink = temp.rlink; // 후속 노드의 주소를 선행 노드의 링크에 저장
								temp.rlink.llink = temp.llink; // 선행 노드의 주소를 후속 노드의 링크에 저장
							}
						}
